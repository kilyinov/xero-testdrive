...
import au.com.community.library.config.KafkaProperties;
...
class CommUnityLibraryKafkaResourceIT {
    ...
    void setup() {
        ...
        CommUnityLibraryKafkaResource kafkaResource = new CommUnityLibraryKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/comm-unity-library-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/comm-unity-library-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
